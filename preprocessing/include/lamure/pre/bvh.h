...
public:
    ...
    { return state_; }
    uint8_t fan_factor() const
    { return fan_factor_; }
    uint32_t depth() const
    { return depth_; }
    size_t max_surfels_per_node() const
    { return max_surfels_per_node_; }
    vec3r translation() const
    { return translation_; }

    boost::filesystem::path base_path() const
    { return base_path_; }

    const std::vector<bvh_node> &nodes() const
    { return nodes_; }
    ...
    surfel_vector get_resampled_leaf_lv_surfels() const
    { return resampled_leaf_level_; }
    ...
    const node_id_type first_leaf() const
    { return first_leaf_; }
...
protected:
    ...
    void set_depth(const uint32_t depth)
    { depth_ = depth; };
    void set_fan_factor(const uint8_t fan_factor)
    { fan_factor_ = fan_factor; };
    ...
    void set_base_path(const boost::filesystem::path &base_path)
    { base_path_ = base_path; };
    void set_nodes(const std::vector<bvh_node> &nodes)
    { nodes_ = nodes; };
    void set_first_leaf(const node_id_type first_leaf)
