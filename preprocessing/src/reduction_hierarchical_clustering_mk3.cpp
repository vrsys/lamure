...
#include <lamure/pre/reduction_hierarchical_clustering_mk3.h>
...
reduction_hierarchical_clustering_mk3::
reduction_hierarchical_clustering_mk3()
{
}
           ...
           const size_t start_node_id) const
{
    if (input[0]->has_provenance()) {
      throw std::runtime_error("reduction_hierarchical_clustering_mk3 not supported for PROVENANCE");
    }
...
split_point_cloud(const std::vector<surfel *> &input_surfels, uint32_t max_cluster_size, real max_variation, const uint32_t &max_clusters) const
{
    std::priority_queue<hierarchical_cluster_mk3,
                        ...
                        cluster_comparator_mk3> cluster_queue;
    cluster_queue.push(calculate_cluster_data(input_surfels));

    while (cluster_queue.size() < max_clusters) {
        hierarchical_cluster_mk3 current_cluster = cluster_queue.top();
        cluster_queue.pop();

        if (current_cluster.surfels.size() > max_cluster_size || current_cluster.variation_pos > max_variation) {
            ...
            if (new_surfels_one.size() > 0) {
                split_cluster_by_color(calculate_cluster_data(new_surfels_one), 0.5, cluster_queue);
            }
            if (new_surfels_two.size() > 0) {
                split_cluster_by_color(calculate_cluster_data(new_surfels_two), 0.5, cluster_queue);
            }
        }
        else {
