...
class COMMON_DLL bounding_box
{
  public:
    ...
    explicit bounding_box(const vec3r& min, const vec3r& max) : min_(min), max_(max)
    {
    ...
    const bool is_invalid() const { return min_.x > max_.x || min_.y > max_.y || min_.z > max_.z; }
    ...
    {
        assert(is_valid());
        return min_.x <= point.x && point.x <= max_.x && min_.y <= point.y && point.y <= max_.y && min_.z <= point.z && point.z <= max_.z;
    }
    ...
    const bool contains(const bounding_box& bounding_box) const
    {
        ...
        return contains(bounding_box.min()) && contains(bounding_box.max());
    }
    ...
    const bool intersects(const bounding_box& bounding_box) const
    {
        assert(is_valid());
        assert(bounding_box.is_valid());
        return !(max_.x < bounding_box.min().x || max_.y < bounding_box.min().y || max_.z < bounding_box.min().z || min_.x > bounding_box.max().x || min_.y > bounding_box.max().y ||
    ...
    inline bool operator==(const bounding_box& rhs) const { return min_ == rhs.min_ && max_ == rhs.max_; }
    inline bool operator!=(const bounding_box& rhs) const { return !(operator==(rhs)); }
