...
#ifndef LAMURE_LOD_CUT_DATABASE_RECORD_H_
#define LAMURE_LOD_CUT_DATABASE_RECORD_H_
...
class LOD_DLL cut_database_record
{
public:
    ...
    void                set_height_divided_by_top_minus_bottom(const view_t view_id, float64_t const height_divided_by_top_minus_bottom);
    void                set_transform(const model_t model_id, const mat4r_t& transform);
    void                set_rendered(const model_t model_id);
    void                set_threshold(const model_t model_id, const float64_t threshold);

    void                receive_cameras(std::map<view_t, camera>& cameras);
    void                receive_height_divided_by_top_minus_bottoms(std::map<view_t, float64_t>& height_divided_by_top_minus_bottoms);
    void                receive_transforms(std::map<model_t, mat4r_t>& transforms);
    void                receive_rendered(std::set<model_t>& rendered);
    void                receive_thresholds(std::map<model_t, float64_t>& thresholds);
...
private:
    ...
    std::map<view_t, float64_t> front_a_height_divided_by_top_minus_bottom_;
    std::map<view_t, float64_t> front_b_height_divided_by_top_minus_bottom_;

    std::map<model_t, mat4r_t> front_a_transforms_;
    std::map<model_t, mat4r_t> front_b_transforms_;

    std::set<model_t> front_a_rendered_;
    std::set<model_t> front_b_rendered_;

    std::map<model_t, float64_t> front_a_thresholds_;
    std::map<model_t, float64_t> front_b_thresholds_;
};
...
#endif // LAMURE_LOD_CUT_DATABASE_RECORD_H_
