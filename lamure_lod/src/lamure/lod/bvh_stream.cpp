...
read_bvh(const std::string& filename, bvh& bvh) {
    ...
    for (const auto& node : nodes) {
       ...
       bvh.set_bounding_box(node.node_id_, lamure::math::bounding_box_t(box_min, box_max));
...
write_bvh(const std::string& filename, bvh& bvh) {
   ...
   tree.reserved_1_ = 0;
   tree.reserved_2_ = 0;
   tree.translation_.x_ = bvh.get_translation().x_;
   tree.translation_.y_ = bvh.get_translation().y_;
   tree.translation_.z_ = bvh.get_translation().z_;
   tree.reserved_3_ = 0;
   ...
   for (uint32_t node_id = 0; node_id < bvh.get_num_nodes(); ++node_id) {
       ...
       const vec3r_t centroid = bvh.get_centroid(node_id);
       node.centroid_.x_ = centroid.x_;
       node.centroid_.y_ = centroid.y_;
       node.centroid_.z_ = centroid.z_;
       ...
       node.reserved_ = 0;
       const lamure::math::bounding_box_t box = bvh.get_bounding_box(node_id);
       node.bounding_box_.min_.x_ = box.min().x_;
       node.bounding_box_.min_.y_ = box.min().y_;
       node.bounding_box_.min_.z_ = box.min().z_;
       node.bounding_box_.max_.x_ = box.max().x_;
       node.bounding_box_.max_.y_ = box.max().y_;
       node.bounding_box_.max_.z_ = box.max().z_;
