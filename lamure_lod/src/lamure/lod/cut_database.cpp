...
void cut_database::
send_height_divided_by_top_minus_bottom(context_t const context_id, view_t const view_id, const float64_t& height_divided_by_top_minus_bottom_value) {
    auto it = records_.find(context_id);
...
void cut_database::
send_transform(context_t const context_id, model_t const model_id, const mat4r_t& transform) {
    auto it = records_.find(context_id);
...
void cut_database::
send_threshold(context_t const context_id, model_t const model_id, const float64_t threshold) {
    auto it = records_.find(context_id);
...
void cut_database::
receive_height_divided_by_top_minus_bottoms(const context_t context_id, std::map<view_t, float64_t>& height_divided_by_top_minus_bottom) {
    auto it = records_.find(context_id);
...
void cut_database::
receive_transforms(const context_t context_id, std::map<model_t, mat4r_t>& transforms) {
    auto it = records_.find(context_id);
...
void cut_database::
receive_thresholds(const context_t context_id, std::map<model_t, float64_t>& thresholds) {
    auto it = records_.find(context_id);
