...
void cut_database_record::
set_height_divided_by_top_minus_bottom(const view_t view_id, float64_t const height_divided_by_top_minus_bottom) {
#ifdef LAMURE_DATABASE_SAFE_MODE
    std::lock_guard<std::mutex> lock(mutex_);
#endif
    if (current_front_ == record_front::FRONT_A) {
...
void cut_database_record::
set_transform(const view_t model_id, const mat4r_t& transform) {
#ifdef LAMURE_DATABASE_SAFE_MODE
    std::lock_guard<std::mutex> lock(mutex_);
#endif
    if (current_front_ == record_front::FRONT_A) {
...
void cut_database_record::
set_threshold(const model_t model_id, const float64_t threshold) {
#ifdef LAMURE_DATABASE_SAFE_MODE
    std::lock_guard<std::mutex> lock(mutex_);
#endif
    if (current_front_ == record_front::FRONT_A) {
...
void cut_database_record::
receive_transforms(std::map<model_t, mat4r_t>& transforms) {
    //transforms.clear();
    ...
    if (current_front_ == record_front::FRONT_A) {
        for (const auto& trans_it : front_b_transforms_) {
            mat4r_t transform = trans_it.second;
...
void cut_database_record::
receive_height_divided_by_top_minus_bottoms(std::map<view_t, float64_t>& height_divided_by_top_minus_bottoms) {
    ...
    if (current_front_ == record_front::FRONT_A) {
        for (const auto& hdtmb_it : front_b_height_divided_by_top_minus_bottom_) {
            ...
            float64_t hdtmb = hdtmb_it.second;
...
void cut_database_record::
receive_thresholds(std::map<model_t, float64_t>& thresholds) {
    //thresholds.clear();
...
expand_front_a(const view_t view_id, const model_t model_id) {
    while (model_id >= front_a_cuts_.size())
    {
        ...
        front_a_transforms_.insert(std::make_pair(model_id, mat4r_t::identity()));
    ...
    while (view_id >= front_a_cuts_[model_id].size())
    {
        ...
        front_a_height_divided_by_top_minus_bottom_.insert(std::make_pair(new_view_id, 1000.0));
        ++new_view_id;
    }
...
expand_front_b(const view_t view_id, const model_t model_id) {
    while (model_id >= front_b_cuts_.size())
    {
        ...
        front_b_transforms_.insert(std::make_pair(model_id, mat4r_t::identity()));
