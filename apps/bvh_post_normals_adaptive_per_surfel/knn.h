...
static scm::math::vec3f get_point_of_interest(
    ...
    surfel* surf = new surfel();
    size_t node_size_in_bytes = bvh->get_primitives_per_node() * sizeof(surfel);
    lod_access->read((char*)surf, (size_t)node_id * node_size_in_bytes + (splat_id*sizeof(surfel)), sizeof(surfel));

    return scm::math::vec3f(surf->x, surf->y, surf->z);
