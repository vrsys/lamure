...
int main(int argc, char *argv[])
{
    ...
    if (get_cmd_option(argv, argv + argc, "-p") != nullptr && get_cmd_option(argv, argv + argc, "-c") != nullptr)
    {
        ...
        boost::filesystem::path path_raster(file_raster);
        boost::filesystem::path dir_raster = path_raster.parent_path();

        Preprocessor preprocessor(dir_raster.c_str(), file_config.c_str());
