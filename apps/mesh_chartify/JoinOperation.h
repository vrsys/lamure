...
public:
  ...
  uint32_t get_chart1_id(){
    return chart1_id;
  }
  uint32_t get_chart2_id(){
    return chart2_id;
  }
  ...
  static bool sort_join_ptrs (std::shared_ptr<JoinOperation> j1, std::shared_ptr<JoinOperation> j2) {
    return (j1->cost < j2->cost);
  }

  static bool compare (std::shared_ptr<JoinOperation> j1, std::shared_ptr<JoinOperation> j2){
    return (j1->get_chart1_id() == j2->get_chart1_id()
            && j1->get_chart2_id() == j2->get_chart2_id());
  }
