...
void Renderer::init(char** argv, scm::shared_ptr<scm::gl::render_device> device, int width_window, int height_window)
{
    ...
    _program_points_dense = device->create_program(
        boost::assign::list_of
            (device->create_shader(scm::gl::STAGE_VERTEX_SHADER, visibility_vs_source))
...
void Renderer::draw_points_dense(Scene scene)
{
    ...
    _context->bind_vertex_array(
    controller->get_context_memory(context_id, lamure::ren::bvh::primitive_type::POINTCLOUD, _device));
    ...
    {
        uint32_t node_culling_result = _camera->cull_against_frustum(
            frustum_by_model,
