    ...
    for(unsigned int i = 0; i < ncam; ++i)
        {
            ...
            images[i] = Image::read_from_file(path_project.string() + token.substr(2, token.size() - 2));
...
{
    boost::filesystem::path path_project(file_path);
    return path_project.parent_path().parent_path();
}
