...
void Scene::init(scm::shared_ptr<scm::gl::render_device> device)
{
    ...
    scm::gl::buffer_ptr vertex_buffer_object_cameras =
        device->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, (sizeof(double) * 3) * vector_struct_camera.size(), &vector_struct_camera[0]);

    _vertex_array_object_cameras = device->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3D, sizeof(double) * 3), boost::assign::list_of(vertex_buffer_object_cameras));
    device->main_context()->apply();

    // create buffer for the cameras
    std::vector<Struct_Image> vector_struct_image = convert_images_to_struct_image();

    scm::gl::buffer_ptr vertex_buffer_object_images =
        device->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, (sizeof(float) * 3) * vector_struct_image.size(), &vector_struct_image[0]);

    _vertex_array_object_images = device->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3D, sizeof(float) * 3), boost::assign::list_of(vertex_buffer_object_images));
    device->main_context()->apply();
}
...
scm::gl::vertex_array_ptr Scene::get_vertex_array_object_images() { return _vertex_array_object_images; }
