...
void Camera::set_still_image(const Image &_still_image) { Camera::_still_image = _still_image; }
...
void Camera::set_orientation(const quat<double> &_orientation) { Camera::_orientation = _orientation; }

const vec3d &Camera::get_center() const { return _center; }

void Camera::set_center(const vec3d &_center) { Camera::_center = _center; }
