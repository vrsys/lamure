...
public:
  ...
  void operator()( HDS& hds) {
    ...
    if (CHECK_VERTICES)
    {
      ...
      if ( (vertices.size()/3) != (tcoords.size() / 2)) {std::cerr << "Input Texture coords not matched with input\n";
          vertices_match = false;}
      else {
        for (uint32_t i = 0; i < tindices.size(); ++i)
        {
          if (tindices[i] != tris[i]){
            std::cerr << "Input Texture coords not matched with input\n";
            vertices_match = false;
            break;
          }
        }
    ...
    if (!vertices_match)
    {
      ...
      for (uint32_t i = 0; i < vertices.size() / 3; ++i)
      {
        ...
        {
          if (tris[t] == vertex_index)
          {
            int target_face_vertex = t;
            int matching_tex_coord_idx = tindices[target_face_vertex] - 1;
      ...
      if ((vertices.size()/3) == (tcoords.size() / 2)){
        ...
        std::cout << "failed to matched texture coords with vertices\n";
      }
