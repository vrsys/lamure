...
void apply_cut_update()
{
    ...
    for(vt::cut_map_entry_type cut_entry : (*cut_db->get_cut_map()))
    {
        ...
        for(auto position_slot_updated : cut->get_front()->get_mem_slots_updated())
        {
            ...
            context_->update_sub_texture(vt_.physical_texture_, scm::gl::texture_region(origin, dimensions), 0, Utils::get_tex_format(), mem_slot_updated->pointer);
        }
...
void init_render_states()
{
    ...
    color_no_blending_state_ = device_->create_blend_state(true, scm::gl::FUNC_SRC_COLOR, scm::gl::FUNC_ONE_MINUS_SRC_ALPHA, scm::gl::FUNC_SRC_ALPHA, scm::gl::FUNC_ONE_MINUS_SRC_ALPHA);

    culling_rasterizer_state_ = device_->create_rasterizer_state(scm::gl::FILL_SOLID, scm::gl::CULL_BACK, scm::gl::ORIENT_CCW, true);
    no_backface_culling_rasterizer_state_ = device_->create_rasterizer_state(scm::gl::FILL_SOLID, scm::gl::CULL_NONE, scm::gl::ORIENT_CCW, true);
...
void init_vt_system()
{
    ...
    vt_.physical_texture_ = device_->create_texture_2d(physical_texture_size, Utils::get_tex_format(), 1, vt::VTConfig::get_instance().get_phys_tex_layers() + 1);
...
void init_camera(const vector<vertex>& vertices)
{
    ...
    camera_ = new lamure::ren::camera(0, make_look_at_matrix(center + scm::math::vec3f(0.f, 0.1f, -0.01f), center, scm::math::vec3f(0.f, 1.f, 0.f)), 5.f, false, false);
