...
std::string lod_vt_mesh_texture_path_ = "";
...
struct vt_info
{
    uint32_t provenance_texture_id_;
    uint32_t model_texture_id_;
    uint16_t view_id_;
    uint16_t context_id_;
    uint64_t provenance_cut_id_;
    uint64_t model_cut_id_;
    vt::CutUpdate* cut_update_;

    std::vector<scm::gl::texture_2d_ptr> model_index_texture_hierarchy_;
    std::vector<scm::gl::texture_2d_ptr> provenance_index_texture_hierarchy_;
...
scm::math::mat4d load_matrix(const std::string& filename)
{
    ...
    if(!file.is_open())
    {
        std::cerr << "Unable to open transformation file: \"" << filename << "\"\n";
...
std::string const strip_whitespace(std::string const& in_string) { return boost::regex_replace(in_string, boost::regex("^ +| +$|( ) +"), "$1"); }
...
void apply_model_vt_cut_update()
{
    ...
    for(vt::cut_map_entry_type cut_entry : (*cut_db->get_cut_map()))
    {
        ...
        for(auto position_slot_updated : cut->get_front()->get_mem_slots_updated())
        {
            ...
            context_->update_sub_texture(vt_.physical_texture_, scm::gl::texture_region(origin, dimensions), 0, get_tex_format(), mem_slot_updated->pointer);
        }
        ...
        for(uint16_t updated_level : updated_levels)
        {
            ...
            context_->update_sub_texture(
                vt_.model_index_texture_hierarchy_.at(updated_level), scm::gl::texture_region(origin, dimensions), 0, scm::gl::FORMAT_RGBA_8UI, cut->get_front()->get_index(updated_level));
        }
...
void apply_provenance_vt_cut_update()
{
    auto* cut_db = &vt::CutDatabase::get_instance();
    ...
    for(vt::cut_map_entry_type cut_entry : (*cut_db->get_cut_map()))
    {
        ...
        for(uint16_t updated_level : updated_levels)
        {
            ...
            context_->update_sub_texture(
                vt_.provenance_index_texture_hierarchy_.at(updated_level), scm::gl::texture_region(origin, dimensions), 0, scm::gl::FORMAT_RGBA_8UI, cut->get_front()->get_index(updated_level));
        }
...
void draw_resources(const lamure::context_t context_id, const lamure::view_t view_id, bool feedback_enabled)
{
    if(sparse_resources_.size() > 0)
    {
        ...
        if(settings_.show_photos_ && !settings_.atlas_file_.empty())
        {
            ...
            uint64_t color_cut_id = vt_.provenance_cut_id_;
            ...
            for(uint32_t i = 0; i < vt_.provenance_index_texture_hierarchy_.size(); ++i)
            {
            ...
            apply_provenance_vt_cut_update();

            for(uint16_t i = 0; i < vt_.provenance_index_texture_hierarchy_.size(); ++i)
            {
                context_->bind_texture(vt_.provenance_index_texture_hierarchy_.at(i), vt_filter_nearest_, i);
            }
...
void draw_all_models(const lamure::context_t context_id, const lamure::view_t view_id, scm::gl::program_ptr shader, lamure::ren::bvh::primitive_type _type)
{
    ...
    {
        context_->bind_vertex_array(controller->get_context_memory(context_id, _type, device_, data_provenance_));
    }
    else
    {
            ...
            if(node_culling_result != 1)
            {
                ...
                if(draw)
                {
                    ...
                    case lamure::ren::bvh::primitive_type::TRIMESH:
                        context_->draw_arrays(scm::gl::PRIMITIVE_TRIANGLE_LIST, (node_slot_aggregate.slot_id_) * (GLsizei)surfels_per_node, surfels_per_node);
                        break;
...
bool parse_prefix(std::string& in_string, std::string const& prefix)
{
    ...
    bool prefix_found = (!(in_string.size() < num_prefix_characters) && strncmp(in_string.c_str(), prefix.c_str(), num_prefix_characters) == 0);
...
bool read_shader(std::string const& path_string, std::string& shader_string, bool keep_optional_shader_code = false)
{
...
void glut_display()
{
    ...
    if(!lod_vt_mesh_texture_path_.empty() || (settings_.show_photos_ && !settings_.atlas_file_.empty()))
    {
        feedback_enabled = vt_.cut_update_->can_accept_feedback(vt_.context_id_);
    ...
    else
    {
        ...
        if(!lod_vt_mesh_texture_path_.empty())
        {
            //selected_single_pass_shading_program = vis_vt_shader_;

            context_->apply();

            selected_single_pass_shading_program->uniform("enable_feedback", feedback_enabled);

            uint64_t color_cut_id = vt_.model_cut_id_;
            uint32_t max_depth_level_color = (*vt::CutDatabase::get_instance().get_cut_map())[color_cut_id]->get_atlas()->getDepth() - 1;

            scm::math::mat4f view_matrix = camera_->get_view_matrix();
            scm::math::mat4f projection_matrix = scm::math::mat4f(camera_->get_projection_matrix());

            selected_single_pass_shading_program->uniform("model_view_matrix", view_matrix);
            selected_single_pass_shading_program->uniform("projection_matrix", projection_matrix);

            selected_single_pass_shading_program->uniform("physical_texture_dim", vt_.physical_texture_size_);
            selected_single_pass_shading_program->uniform("max_level", max_depth_level_color);
            selected_single_pass_shading_program->uniform("tile_size", scm::math::vec2((uint32_t)vt::VTConfig::get_instance().get_size_tile()));
            selected_single_pass_shading_program->uniform("tile_padding", scm::math::vec2((uint32_t)vt::VTConfig::get_instance().get_size_padding()));

            selected_single_pass_shading_program->uniform("enable_hierarchy", vt_.enable_hierarchy_);
            selected_single_pass_shading_program->uniform("toggle_visualization", vt_.toggle_visualization_);

            for(uint32_t i = 0; i < vt_.model_index_texture_hierarchy_.size(); ++i)
            {
                std::string texture_string = "hierarchical_idx_textures";
                selected_single_pass_shading_program->uniform(texture_string, i, int((i)));
            }

            selected_single_pass_shading_program->uniform("physical_texture_array", 17);
            ...
            apply_model_vt_cut_update();

            for(uint16_t i = 0; i < vt_.model_index_texture_hierarchy_.size(); ++i)
            {
                context_->bind_texture(vt_.model_index_texture_hierarchy_.at(i), vt_filter_nearest_, i);
            }
        ...
        draw_resources(context_id, view_id, feedback_enabled);
    }
...
void brush()
{
    ...
    if(hit)
    {
        ...
        xyz xyz{intersection.position_ + intersection.normal_ * settings_.aux_point_distance_,
                (uint8_t)color.x,
                (uint8_t)color.y,
                (uint8_t)color.z,
                (uint8_t)255,
                    ...
                    if(selected_node_id > 0)
                    {
                        ...
                        if(settings_.show_photos_)
                        {
                            ...
                            for(const auto img : imgs)
                            {
                                ...
                                scm::math::vec3f direction_feature = scm::math::normalize(scm::math::vec3f(xyz.pos_.x, xyz.pos_.y, xyz.pos_.z) - view_translation);
                                scm::math::vec3f direction_view = scm::math::normalize(scm::math::vec3f(view.transform_[8], view.transform_[9], view.transform_[10]));
...
void glut_keyboard(unsigned char key, int32_t x, int32_t y)
{
    ...
    case 'F':
    {
        ...
        settings_.travel_speed_ = (settings_.travel_ == 0 ? 0.5f : settings_.travel_ == 1 ? 5.5f : settings_.travel_ == 2 ? 20.5f : settings_.travel_ == 3 ? 100.5f : 300.5f);
    ...
    case '0':
        ...
        break;
    case '-':
        if(--selection_.selected_model_ < 0)
            selection_.selected_model_ = num_models_ - 1;
        break;
    case '=':
        if(++selection_.selected_model_ >= num_models_)
            selection_.selected_model_ = 0;
        break;
    ...
    if(settings_.pvs_ != "")
    {
        ...
        if(settings_.create_aux_resources_)
        {
            if(pvs->get_visibility_grid() != nullptr)
            {
                ...
                pvs_resource_.buffer_ = device_->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, (sizeof(float) * 3) * pvs_lines_to_upload.size(), &pvs_lines_to_upload[0]);
                pvs_resource_.array_ = device_->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3F, sizeof(float) * 3), boost::assign::list_of(pvs_resource_.buffer_));
    ...
    for(const auto& aux_file : settings_.aux_)
    {
        if(aux_file.second != "")
        {
            ...
            for(uint32_t i = 0; i < aux.get_num_views(); ++i)
            {
                ...
                points_to_upload.push_back(xyz{view.position_, (uint8_t)255, (uint8_t)240, (uint8_t)0, (uint8_t)255, settings_.aux_point_size_, scm::math::vec3f(1.0, 0.0, 0.0)} // placeholder
                );
            ...
            for(uint32_t i = 0; i < aux.get_num_sparse_points(); ++i)
            {
                ...
                points_to_upload.push_back(xyz{point.pos_, point.r_, point.g_, point.b_, point.a_, settings_.aux_point_size_, scm::math::vec3f(1.0, 0.0, 0.0)} // placeholder
                );
            }
            ...
            points_resource.buffer_ = device_->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, sizeof(xyz) * points_to_upload.size(), &points_to_upload[0]);
            points_resource.array_ =
                device_->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3F, sizeof(xyz))(0, 1, scm::gl::TYPE_UBYTE, sizeof(xyz), scm::gl::INT_FLOAT_NORMALIZE)(
                                                 0, 2, scm::gl::TYPE_UBYTE, sizeof(xyz), scm::gl::INT_FLOAT_NORMALIZE)(0, 3, scm::gl::TYPE_UBYTE, sizeof(xyz), scm::gl::INT_FLOAT_NORMALIZE)(
                                                 0, 4, scm::gl::TYPE_UBYTE, sizeof(xyz), scm::gl::INT_FLOAT_NORMALIZE)(0, 5, scm::gl::TYPE_FLOAT, sizeof(xyz))(0, 6, scm::gl::TYPE_VEC3F, sizeof(xyz)),
            ...
            octree_resource.buffer_ = device_->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, (sizeof(float) * 3) * octree_lines_to_upload.size(), &octree_lines_to_upload[0]);
            octree_resource.array_ = device_->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3F, sizeof(float) * 3), boost::assign::list_of(octree_resource.buffer_));
            ...
            if(!settings_.atlas_file_.empty())
            {
                ...
                {
                    throw std::runtime_error("Number of atlas_tiles (" + std::to_string(aux.get_num_atlas_tiles()) + ") " + "does not match number of views (" + std::to_string(aux.get_num_views()) +
                                             ")");
                }
                ...
                triangles_resource.buffer_ = device_->create_buffer(scm::gl::BIND_VERTEX_BUFFER, scm::gl::USAGE_STATIC_DRAW, (sizeof(vertex)) * triangles_to_upload.size(), &triangles_to_upload[0]);

                triangles_resource.array_ = device_->create_vertex_array(scm::gl::vertex_format(0, 0, scm::gl::TYPE_VEC3F, sizeof(vertex))(0, 1, scm::gl::TYPE_VEC2F, sizeof(vertex)),
...
struct Window
{
    Window() { _mouse_button_state = MouseButtonState::IDLE; }
...
class EventHandler
{
  public:
    ...
    static void on_window_enter(GLFWwindow* glfw_window, int entered) { Window* window = (Window*)glfwGetWindowUserPointer(glfw_window); }
};
...
void init_camera()
{
    ...
    if(num_models_ > 0 && settings_.models_[0].substr(settings_.models_[0].size() - 3) == "bvh")
    {
    ...
    camera_ = new lamure::ren::camera(0, make_look_at_matrix(center + scm::math::vec3f(0.f, 0.1f, -0.01f), center, scm::math::vec3f(0.f, 1.f, 0.f)), length(root_bb_max - root_bb_min), false, false);
...
{
    vt::VTConfig::CONFIG_PATH = "/mnt/terabytes_of_textures/FINAL_DEMO_DATA/config_demo_do_not_modify.ini"; // settings_.atlas_file_.substr(0, settings_.atlas_file_.size() - 5) + "ini";
    ...
    if(!settings_.atlas_file_.empty())
    {
        vt_.provenance_texture_id_ = vt::CutDatabase::get_instance().register_dataset(settings_.atlas_file_);
        vt_.provenance_cut_id_ = vt::CutDatabase::get_instance().register_cut(vt_.provenance_texture_id_, vt_.view_id_, vt_.context_id_);
    }

    if(!lod_vt_mesh_texture_path_.empty())
    {
        vt_.model_texture_id_ = vt::CutDatabase::get_instance().register_dataset(lod_vt_mesh_texture_path_);
        vt_.model_cut_id_ = vt::CutDatabase::get_instance().register_cut(vt_.model_texture_id_, vt_.view_id_, vt_.context_id_);
    }
...
void init_vt_system()
{
    ...
    // add_data
    uint16_t depth = (uint16_t)((*vt::CutDatabase::get_instance().get_cut_map())[vt_.model_cut_id_]->get_atlas()->getDepth());
    uint16_t level = 0;
    ...
    while(level < depth)
    {
        ...
        vt_.model_index_texture_hierarchy_.emplace_back(index_texture_level_ptr);
    ...
    {
        // add_data
        depth = (uint16_t)((*vt::CutDatabase::get_instance().get_cut_map())[vt_.provenance_cut_id_]->get_atlas()->getDepth());
        level = 0;
        ...
        while(level < depth)
        {
            ...
            device_->main_context()->clear_image_data(index_texture_level_ptr, 0, scm::gl::FORMAT_RGBA_8UI, 0);
            vt_.provenance_index_texture_hierarchy_.emplace_back(index_texture_level_ptr);
    ...
    vt_.physical_texture_ = device_->create_texture_2d(physical_texture_size, get_tex_format(), 1, vt::VTConfig::get_instance().get_phys_tex_layers() + 1);
...
void init()
{
    ...
    try
    {
        ...
        if(!read_shader(shader_root_path + "/vis/vis_quad.glslv", vis_quad_vs_source) || !read_shader(shader_root_path + "/vis/vis_quad.glslf", vis_quad_fs_source) ||
           !read_shader(shader_root_path + "/vis/vis_line.glslv", vis_line_vs_source) || !read_shader(shader_root_path + "/vis/vis_line.glslf", vis_line_fs_source) ||
           !read_shader(shader_root_path + "/vis/vis_trimesh.glslv", vis_trimesh_vs_source) || !read_shader(shader_root_path + "/vis/vis_trimesh.glslf", vis_trimesh_fs_source) ||
           ...
           || !read_shader(shader_root_path + "/vt/virtual_texturing.glslv", vis_vt_vs_source) || !read_shader(shader_root_path + "/vt/virtual_texturing_hierarchical.glslf", vis_vt_fs_source) ||
           !read_shader(shader_root_path + "/vt/virtual_texturing_inv_index_generation.glslv", vis_vt_vs_feedback_source)

           || !read_shader(shader_root_path + "/vis/vis_xyz.glslv", vis_xyz_vs_source) || !read_shader(shader_root_path + "/vis/vis_xyz.glslg", vis_xyz_gs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz.glslf", vis_xyz_fs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_pass1.glslv", vis_xyz_pass1_vs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz_pass1.glslg", vis_xyz_pass1_gs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_pass1.glslf", vis_xyz_pass1_fs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz_pass2.glslv", vis_xyz_pass2_vs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_pass2.glslg", vis_xyz_pass2_gs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz_pass2.glslf", vis_xyz_pass2_fs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_pass3.glslv", vis_xyz_pass3_vs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz_pass3.glslf", vis_xyz_pass3_fs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_qz.glslv", vis_xyz_qz_vs_source) ||
           !read_shader(shader_root_path + "/vis/vis_xyz_qz_pass1.glslv", vis_xyz_qz_pass1_vs_source) || !read_shader(shader_root_path + "/vis/vis_xyz_qz_pass2.glslv", vis_xyz_qz_pass2_vs_source)

           || !read_shader(shader_root_path + "/vis/vis_xyz.glslv", vis_xyz_vs_lighting_source, true) || !read_shader(shader_root_path + "/vis/vis_xyz.glslg", vis_xyz_gs_lighting_source, true) ||
        ...
        vis_quad_shader_ = device_->create_program(
            boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_quad_vs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_quad_fs_source)));
        if(!vis_quad_shader_)
        {
        ...
        vis_line_shader_ = device_->create_program(
            boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_line_vs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_line_fs_source)));
        if(!vis_line_shader_)
        {
        ...
        vis_trimesh_shader_ = device_->create_program(
            boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_trimesh_vs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_trimesh_fs_source)));
        ...
        vis_trimesh_lighting_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_trimesh_vs_lighting_source))(
        ...
        vis_vt_shader_ = device_->create_program(
            boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_vt_vs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_vt_fs_source)));
        if(!vis_vt_shader_)
        {
        ...
        vis_vt_shader_feedback_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_vt_vs_feedback_source)));
        ...
        vis_xyz_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_vs_source))(
            device_->create_shader(scm::gl::STAGE_GEOMETRY_SHADER, vis_xyz_gs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_fs_source)));
        if(!vis_xyz_shader_)
        {
        ...
        vis_xyz_pass1_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_pass1_vs_source))(
            device_->create_shader(scm::gl::STAGE_GEOMETRY_SHADER, vis_xyz_pass1_gs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_pass1_fs_source)));
        ...
        vis_xyz_pass2_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_pass2_vs_source))(
            device_->create_shader(scm::gl::STAGE_GEOMETRY_SHADER, vis_xyz_pass2_gs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_pass2_fs_source)));
        ...
        vis_xyz_pass3_shader_ = device_->create_program(
            boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_pass3_vs_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_pass3_fs_source)));
        ...
        vis_xyz_lighting_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_vs_lighting_source))(
            device_->create_shader(scm::gl::STAGE_GEOMETRY_SHADER, vis_xyz_gs_lighting_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_fs_lighting_source)));
        ...
        vis_xyz_pass2_lighting_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_pass2_vs_lighting_source))(
            device_->create_shader(scm::gl::STAGE_GEOMETRY_SHADER, vis_xyz_pass2_gs_lighting_source))(device_->create_shader(scm::gl::STAGE_FRAGMENT_SHADER, vis_xyz_pass2_fs_lighting_source)));
        ...
        vis_xyz_pass3_lighting_shader_ = device_->create_program(boost::assign::list_of(device_->create_shader(scm::gl::STAGE_VERTEX_SHADER, vis_xyz_pass3_vs_lighting_source))(
    ...
    if(!settings_.atlas_file_.empty() || !lod_vt_mesh_texture_path_.empty())
    {
        init_vt_database();
        init_vt_system();
    }
...
void gui_selection_settings(settings& stgs)
{
    ...
    if(settings_.create_aux_resources_ && settings_.atlas_file_ != "")
    {
        ...
        {
            ImGui::Text("Image: %d %s", (int32_t)selection_.selected_view_, settings_.views_[selection_.selected_model_][selection_.selected_view_].image_file_.c_str());
        }
        else
        {
...
void gui_visual_settings()
{
    ...
    const char* vis_values[] = {
        "Color", "Normals", "Accuracy", "Radius Deviation", "Output Sensitivity", "Provenance 1", "Provenance 2", "Provenance 3", "Provenance 4", "Provenance 5", "Provenance 6", "Provenance 7"};
...
int main(int argc, char* argv[])
{
    ...
    if(argc >= 2)
    {
        ...
        if(argc > 3)
        {
            lod_vt_mesh_texture_path_ = std::string(argv[3]);
            std::cout << "VT: " << lod_vt_mesh_texture_path_ << std::endl;
        }
    }
    else
    {
    ...
    settings_.vis_ = settings_.show_normals_ ? 1 : settings_.show_accuracy_ ? 2 : settings_.show_output_sensitivity_ ? 3 : settings_.channel_ > 0 ? 3 + settings_.channel_ : 0;
