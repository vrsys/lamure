...
std::pair<vec3ui_t, vec3b_t> reduction_normal_deviation_clustering::
    ...
    if (!bb_zero_size) { // at least 1 dimesion
        ...
        while (true) {
            ...
            if (termination_ctr++ >=40000) {
                LAMURE_LOG_WARN("Reached maximum number of iterations in ndc while computing grid dimensions!");
                break;
            }
            ...
            vec3r_t cell_size = vec3r_t(fabs(bb_dimensions[0]/grid_dimensions[0]),
            ...
            for (uint32_t i = 0; i < input.size(); ++i)
            {
                ...
                {
                    vec3r_t surfel_pos = input[i]->read_surfel_ref(j).pos() - bounding_box.min();
          ...
          const size_t start_node_id) const
{
    ...
    for (uint32_t i = 0; i < input.size(); ++i)
    {
        for (uint32_t j = 0; j < input[i]->length(); ++j)
        {
            vec3r_t surfel_pos =  input[i]->read_surfel_ref(j).pos() - bbox.min();
            if (surfel_pos.x_ < 0.f) surfel_pos.x_ = 0.f;
            if (surfel_pos.y_ < 0.f) surfel_pos.y_ = 0.f;
            if (surfel_pos.z_ < 0.f) surfel_pos.z_ = 0.f;
    ...
    while (surfel_count > surfels_per_node)
    {
        ...
        if (termination_ctr++ >=30000) {
            LAMURE_LOG_WARN("Reached maximum number of iterations in ndc!"
