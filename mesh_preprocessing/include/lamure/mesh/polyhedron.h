...
struct XtndPoint : public Traits::Point_3
{
    ...
    XtndPoint(double x, double y, double z) : Traits::Point_3(x, y, z), texCoord(0.0, 0.0) {}

    XtndPoint(double x, double y, double z, double u, double v) : Traits::Point_3(x, y, z), texCoord(u, v) {}
    ...
    void add_uv_candidate(UV_Candidate uvc) { uv_candidates.push_back(uvc); }
    ...
    static bool float_safe_equals(Point p1, Point p2)
    {
        ...
        if(std::abs(p1.x() - p2.x()) > eps || std::abs(p1.y() - p2.y()) > eps || std::abs(p1.z() - p2.z()) > eps)
        {
            return false;
        }
        return true;
...
struct Border_is_constrained_edge_map
{
    ...
    friend bool get(Border_is_constrained_edge_map m, const key_type& edge) { return CGAL::is_border(edge, *m.sm_ptr); }
};
...
class polyhedron_builder : public CGAL::Modifier_base<HDS>
{
  public:
    ...
    polyhedron_builder(std::vector<Triangle_Chartid>& _combined_set) : combined_set(_combined_set) {}

    void set_mesh_proportion(const double _mp) { mesh_proportion = _mp; }
    ...
    void create_indexed_triangle_list(std::vector<lamure::mesh::Triangle_Chartid>& input_triangles, std::vector<uint32_t>& tris, std::vector<XtndPoint<Kernel>>& vertices)
    {
        ...
        for(const lamure::mesh::Triangle_Chartid tri : input_triangles)
        {
            ...
            {
                // get Point value
                const XtndPoint<Kernel> tri_pnt = XtndPoint<Kernel>(tri.getVertex(v).pos_.x, tri.getVertex(v).pos_.y, tri.getVertex(v).pos_.z, tri.getVertex(v).tex_.x, tri.getVertex(v).tex_.y);
...
struct OBJ_printer
{
    static void write_vertex(std::ostream& out, const double& x, const double& y, const double& z) { out << "v " << x << ' ' << y << ' ' << z << '\n'; }

    template <class Polyhedron>
    static void print_polyhedron(std::ostream& out, const Polyhedron& P, std::string filename)
    {
        ...
        {
            write_vertex(out, ::CGAL::to_double(vi->point().x()), ::CGAL::to_double(vi->point().y()), ::CGAL::to_double(vi->point().z()));
        }
