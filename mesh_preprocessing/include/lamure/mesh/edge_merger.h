...
struct edge_merger
{
  private:
    ...
    static bool point_equal_to_vec3(lamure::mesh::Point lhs, scm::math::vec3f rhs) { return (lhs.x() == rhs.x && lhs.y() == rhs.y && lhs.z() == rhs.z); }

  public:
    static void merge_similar_border_edges(lamure::mesh::Polyhedron& P, std::vector<lamure::mesh::Triangle_Chartid>& tri_list, int target_merges)
    {
        struct Edge_merge_candidate
        {
            ...
            static bool sort_edge_candidates(Edge_merge_candidate e1, Edge_merge_candidate e2) { return (e1.cost < e2.cost); }
        };
        ...
        std::cout << blocked_same_tri << " shared a face, " << blocked_no_adjacent_face << "  did not have adjacent faces, " << continuing_edge_error << " had continuing edge problem ( "
                  << at_least_1_edge << " had at least 1 edge)\n";
        ...
        while(merges_executed < target_merges && !merge_cs.empty())
        {
            ...
            for(int i = 0; i < 3; ++i)
            {
                ...
                if(new_tri_vertices.size() >= 3)
                {
                    break;
                } // max size of 3 at this point
            }
            ...
            std::cout << "created triangle "
                      << " [" << merge.edge1->edge_id << ", " << merge.edge2->edge_id << "]"
                      << "\n";
