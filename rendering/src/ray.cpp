		    ...
		    if (surfel.size >= std::numeric_limits<float>::min()) {
                        ...
                        if (intersect_surfel(surfel, object_ray_origin, object_ray_direction, ts)) {
                            ...
                            if (scm::math::length(splat_position - origin_) < max_distance_) {
                                ...
                                float splat_distance = scm::math::length(splat_plane_intersection - origin_);
                                float error = 0.01f * splat_distance + splat_plane_distance;
                                ...
                                if (error < intersection.error_ && error < max_intersection_error) {
                                   ...
                                   has_hit = true;
                                   intersection.distance_ = splat_distance;
...
intersect_aabb(const scm::gl::boxf& bb,
    ...
    scm::math::vec3f tmin1 = scm::math::vec3f(
        std::min(t1.x, t2.x), std::min(t1.y, t2.y), std::min(t1.z, t2.z));
    scm::math::vec3f tmax1 = scm::math::vec3f(
