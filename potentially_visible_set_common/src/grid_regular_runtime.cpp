...
grid_regular_runtime::
grid_regular_runtime(const unsigned int& number_cells, const double& cell_size, const scm::math::vec3d& position_center)
{
...
unsigned int grid_regular_runtime::
...
get_cell_at_index(const unsigned int& index)
{
	...
	/*if(current_cell->get_visibility.size() == ids_.size())
	{
		return current_cell;
	}*/

	// One line per model.get_cell_at_index
	unsigned int single_model_bytes = 0;

	for(model_t model_index = 0; model_index < ids_.size(); ++model_index)
	{
		unsigned int num_nodes = ids_.at(model_index);
