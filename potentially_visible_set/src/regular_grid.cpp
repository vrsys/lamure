...
get_cell_at_position(const scm::math::vec3d& position)
{
	...
	std::cout << "current view cell index: " << general_index << std::endl;

	return get_cell_at_index(general_index);
}

void regular_grid::
save_grid_to_file(std::string file_path)
{
...
void regular_grid::
save_visibility_to_file(std::string file_path)
{
	std::fstream file_out;
...
bool regular_grid::
load_grid_from_file(std::string file_path)
{
	std::fstream file_in;
...
bool regular_grid::
load_visibility_from_file(std::string file_path)
{
	std::fstream file_in;
	...
	for(unsigned int cell_index = 0; cell_index < cells_.size(); ++cell_index)
	{
		...
		{
			unsigned int num_nodes = database->get_model(model_index)->get_bvh()->get_num_nodes();
