...
class FibonacciHeap
{
  ...
  protected:
    ...
    static void _insertBefore(FibonacciHeap<key_type, value_type>* first, FibonacciHeap<key_type, value_type>* last, FibonacciHeap<key_type, value_type>* ref)
    {
        if(ref == nullptr)
        {
            return;
  ...
  public:
    ...
    virtual void setCleanupFrequency(size_t consolidateFreq) { _consolidateFreq = consolidateFreq; }
    ...
    virtual bool pop(value_type& val, std::chrono::milliseconds maxTime = std::chrono::milliseconds::zero())
    {
        ...
        if(_min == nullptr)
        {
            if(!_newEntry.wait_for(lock, maxTime, [this] { return _min != nullptr; }))
            {
                return false;
            }
        }
