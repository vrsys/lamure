...
class AbstractQueueEntry
{
  ...
  public:
    ...
    virtual void setPrev(AbstractQueueEntry<content_type>* prev) { _prev.store(prev); }

    virtual void setNext(AbstractQueueEntry<content_type>* next) { _next.store(next); }

    virtual AbstractQueueEntry<content_type>* getPrev() { return _prev.load(); }

    virtual AbstractQueueEntry<content_type>* getNext() { return _next.load(); }

    virtual content_type getContent() { return _content.load(); }

    virtual void remove() { _queue.load()->remove(*this); }

    virtual AbstractQueue<content_type>* getQueue() { return _queue.load(); }
};
