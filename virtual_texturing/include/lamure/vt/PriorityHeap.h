...
class VT_DLL PriorityHeap : public FibonacciHeap<priority_type, PriorityHeapContent<priority_type>*>
{
  ...
  public:
    explicit PriorityHeap(priority_type key, PriorityHeapContent<priority_type>* value)
        : FibonacciHeap<priority_type, PriorityHeapContent<priority_type>*>(std::numeric_limits<key_type>::max() - key, value)
    {
    }
    ...
    void setValue(value_type value) { this->_value = value; }
};
...
class VT_DLL PriorityHeapContent
{
  ...
  public:
    PriorityHeapContent() { _heap.store(nullptr); }
