...
class AbstractBufferSlot
{
  ...
  protected:
    ...
    explicit AbstractBufferSlot(AbstractBuffer<assoc_data_type> &buffer) : AbstractBufferSlot() { _buffer = &buffer; }
    ...
    virtual void setBuffer(AbstractBuffer<assoc_data_type> &buffer) { _buffer = &buffer; }
  ...
  public:
    ...
    virtual AbstractBuffer<assoc_data_type> &getBuffer() { return *_buffer; }

    virtual STATE getState() { return _state.load(); }

    virtual uint8_t *getPointer() { return _ptr; }
    ...
    assoc_data_type getAssocData() { return _assocData.load(); }

    size_t getSetCount() { return _setAsscoData; }
};
...
{
  public:
    virtual void slotStateChange(AbstractBufferSlot<assoc_data_type> *slot, typename AbstractBufferSlot<assoc_data_type>::STATE oldState,
                                 typename AbstractBufferSlot<assoc_data_type>::STATE newState) = 0;
  ...
  public:
    ...
    virtual bool slotReading(AbstractBufferSlot<assoc_data_type> *slot) { return slot->_setState(AbstractBufferSlot<assoc_data_type>::STATE::READING); }

    virtual bool slotReady(AbstractBufferSlot<assoc_data_type> *slot) { return slot->_setState(AbstractBufferSlot<assoc_data_type>::STATE::READY); }

    virtual bool slotFree(AbstractBufferSlot<assoc_data_type> *slot) { return slot->_setState(AbstractBufferSlot<assoc_data_type>::STATE::FREE); }
};
}
