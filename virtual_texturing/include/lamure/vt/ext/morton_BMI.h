...
{
inline uint32_t pdep(uint32_t source, uint32_t mask) noexcept { return _pdep_u32(source, mask); }
inline uint64_t pdep(uint64_t source, uint64_t mask) noexcept { return _pdep_u64(source, mask); }
inline uint32_t pext(uint32_t source, uint32_t mask) noexcept { return _pext_u32(source, mask); }
inline uint64_t pext(uint64_t source, uint64_t mask) noexcept { return _pext_u64(source, mask); }
} // namespace bmi2_detail
...
{
    morton m = 0;
    m |= bmi2_detail::pdep(static_cast<morton>(x), static_cast<morton>(BMI_2D_X_MASK)) | bmi2_detail::pdep(static_cast<morton>(y), static_cast<morton>(BMI_2D_Y_MASK));
    return m;
}
...
{
    morton m = 0;
    m |= bmi2_detail::pdep(static_cast<morton>(x), static_cast<morton>(BMI_3D_X_MASK)) | bmi2_detail::pdep(static_cast<morton>(y), static_cast<morton>(BMI_3D_Y_MASK)) |
