...
inline morton m3D_e_sLUT(const coord x, const coord y, const coord z)
{
    ...
    for(unsigned int i = sizeof(coord); i > 0; --i)
    {
        ...
        answer = answer << 24 | (Morton3D_encode_z_256[(z >> shift) & EIGHTBITMASK] | Morton3D_encode_y_256[(y >> shift) & EIGHTBITMASK] | Morton3D_encode_x_256[(x >> shift) & EIGHTBITMASK]);
    }
    return answer;
}
...
inline morton m3D_e_LUT(const coord x, const coord y, const coord z)
{
    ...
    for(unsigned int i = sizeof(coord); i > 0; --i)
    {
        unsigned int shift = (i - 1) * 8;
        answer =
            answer << 24 | (Morton3D_encode_x_256[(z >> shift) & EIGHTBITMASK] << 2) | (Morton3D_encode_x_256[(y >> shift) & EIGHTBITMASK] << 1) | Morton3D_encode_x_256[(x >> shift) & EIGHTBITMASK];
    }
    return answer;
}
...
static inline morton morton3D_SplitBy3bits(const coord a)
{
    ...
    if(sizeof(morton) == 8)
    {
        x = (x | (uint_fast64_t)x << 32) & masks[1];
    } // for 64-bit case
    x = (x | x << 16) & masks[2];
    x = (x | x << 8) & masks[3];
    x = (x | x << 4) & masks[4];
...
inline morton m3D_e_for_ET(const coord x, const coord y, const coord z)
{
    ...
    for(unsigned int i = 0; i < checkbits; ++i)
    {
        ...
        answer |= ((x & m_shifted) << shift) | ((y & m_shifted) << (shift + 1)) | ((z & m_shifted) << (shift + 2));
    }
    return answer;
}
