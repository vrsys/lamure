...
inline morton m2D_e_sLUT(const coord x, const coord y)
{
    ...
    for(unsigned int i = sizeof(coord); i > 0; --i)
    {
        ...
        answer = answer << 16 | Morton2D_encode_y_256[(y >> shift) & EIGHTBITMASK] | Morton2D_encode_x_256[(x >> shift) & EIGHTBITMASK];
    }
    return answer;
}
...
inline morton m2D_e_LUT(const coord x, const coord y)
{
    ...
    {
        unsigned int shift = (i - 1) * 8;
        answer = answer << 16 | (Morton2D_encode_x_256[(y >> shift) & EIGHTBITMASK] << 1) | (Morton2D_encode_x_256[(x >> shift) & EIGHTBITMASK]);
    }
    return answer;
}
