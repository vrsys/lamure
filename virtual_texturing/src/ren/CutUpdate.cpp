...
void CutUpdate::dispatch()
{
    ...
    for(auto iter = _cut.get_back_cut().rbegin(); iter != _cut.get_back_cut().rend(); ++iter)
    {
        ...
        if(texels_per_tile < _feedback_buffer[mem_slot] * 2.0f && QuadTree::get_depth_of_node(tile_id) < 3) //_context->get_depth_quadtree())
        {
        ...
        else if(texels_per_tile > (float)sum_feedback / 8.0f && check_all_siblings_in_cut(tile_id) && tile_id > 0)
        {
