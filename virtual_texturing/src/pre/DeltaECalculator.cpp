...
namespace vt {
    namespace pre {
        ...
        void DeltaECalculator::calculate(size_t maxMemory) {
            ...
            size_t distBufferByteSize = _treeDepth * innerTilePxCount;
                ...
                for (uint64_t rootLevelRelId = levelTiles - 1;; --rootLevelRelId) {
                    ...
                    if (rootXCoord < actualLevelTileWidth && rootYCoord < actualLevelTileHeight) {
                        ...
                        if (tileOffset < rootBufferOffset || tileOffset >= (rootBufferOffset + rootBufferByteSize)) {
                            ...
                            _file.read((char *) rootBuffer, std::min(rootBufferByteSize, firstTileInLevelFileOffset +
                                        ...
                                        avrgDist =
                                                ((lastLevelBuffer[(y << 1) * _innerTileWidth + (x << 1)] +
                                                 ...
                                                 (lastLevelBuffer[((y << 1) + 1) * _innerTileWidth + (x << 1)] +
                                                  lastLevelBuffer[((y << 1) + 1) * _innerTileWidth + (x << 1) + 1]) /
                                                 2) / 2;
