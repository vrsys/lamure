...
void Preprocessor::_deflate(size_t writeBufferSize){
    ...
    if(_destLayout == AtlasFile::LAYOUT::RAW){
        writeBufferOffset = offsetAfterLastTile - (QuadTree::firstIdOfLevel(_treeDepth - 1) % writeBufferTileSize) * _destTileByteSize;
    ...
    if(_treeDepth > 1) {
        for (size_t iterationLevel = _treeDepth - 2; /* iterationLevel > 0 */; --iterationLevel) {
            ...
            std::cout << "Deflating " << (levelTileWidth * levelTileHeight) << " Tiles in Level " << (iterationLevel + 1) << std::endl;
            std::cout << std::setw(3) << (int)progress << " %";
            std::cout.flush();
#endif

            for(uint64_t relIterationId = tilesInIterationLevel - 1; /* relIterationId > 0 */; --relIterationId){
                uint64_t x;
                uint64_t y;
                ...
                for(uint8_t relQuadId = 3;; --relQuadId){
                    ...
                    if(childX < levelTileWidth && childY < levelTileHeight) {
                        len = _getTileById(absId, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                    }
                ...
                bufferOffset += _destTileByteSize;
                if(relId1 != relId) len = _getTileById(firstIdOfCurrentLevel + relId2, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);

                bufferOffset += _destTileByteSize;
                if(relId1 != relId) len = _getTileById(firstIdOfCurrentLevel + relId1, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);

                len = 0;

                bufferOffset += _destTileByteSize;
                if(relId1 != relId && relId0 != relId1) len = _getTileById(firstIdOfCurrentLevel + relId0, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);
                ...
                bufferOffset += _destTileByteSize;
                if(relId0 != relId) len = _getTileById(firstIdOfCurrentLevel + relId1, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);

                bufferOffset += _destTileByteSize;
                if(relId0 != relId) len = _getTileById(firstIdOfCurrentLevel + relId0, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);
                ...
                bufferOffset += _destTileByteSize;
                len = _getTileById(firstIdOfIterationLevel + relId2, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);

                bufferOffset += _destTileByteSize;
                len = _getTileById(firstIdOfIterationLevel + relId1, writeBuffer, writeBufferFirstId, writeBufferLastId, idLookup, writeBufferTileSize, &buffer[bufferOffset]);
                if (len == 0) memset_volatile(&buffer[bufferOffset], 0, _destTileByteSize);
                ...
                if(yIsLast) {
                    ...
                    writeBitmap.fillRect(transPx, Bitmap::PIXEL_FORMAT::RGBA8, 0, padHeight + _padding, padWidth + _padding, _tileHeight - padHeight - _padding);
                }else{
                    // pad bottom side
                ...
                if(xIsLast) {
                    ...
                    writeBitmap.fillRect(transPx, Bitmap::PIXEL_FORMAT::RGBA8, padWidth + _padding, 0, _tileWidth - padWidth - _padding, _tileHeight);
                }else {
                    // pad right side
                ...
                if(_destLayout == AtlasFile::LAYOUT::RAW){
                    ...
                    while(currentOffset < writeBufferOffset){
                        ...
                        _destPayloadFile.write((char*)writeBuffer, std::min(writeBufferSize, offsetAfterLastTile - writeBufferOffset));
                    ...
                    std::memset(&writeBuffer[currentOffset - writeBufferOffset + _destTileByteSize], 0x00, lastWriteOffset - currentOffset - _destTileByteSize);
                    std::memcpy(&writeBuffer[currentOffset - writeBufferOffset], (char*)&buffer[bufferOffset], _destTileByteSize);
...
#ifdef PREPROCESSOR_LOG_PROGRESS
                ++tilesWritten;
                auto currentProgress = (uint8_t)(tilesWritten * 100 / iterationLevelTileWidth / iterationLevelTileHeight);
...
size_t Preprocessor::_getTileById(uint64_t id, const uint8_t *buffer, uint64_t firstIdInBuffer, uint64_t lastIdInBuffer, const uint64_t *idLookup, size_t bufferTileLen, uint8_t *out){
    size_t len = _getBufferedTileById(id, buffer, firstIdInBuffer, lastIdInBuffer, idLookup, bufferTileLen, out);
...
size_t Preprocessor::_getBufferedTileById(uint64_t id, const uint8_t *buffer, uint64_t firstIdInBuffer, uint64_t lastIdInBuffer, const uint64_t *idLookup, size_t bufferTileLen, uint8_t *out){
    size_t len = 0;
...
void Preprocessor::_extract(size_t bufferTileWidth, size_t writeBufferSize){
    ...
    if(_destLayout == AtlasFile::LAYOUT::RAW){
        uint64_t tilesInFinestLevel = QuadTree::getWidthOfLevel(_treeDepth - 1) * QuadTree::getWidthOfLevel(_treeDepth - 1);
        writeBufferOffset = offsetAfterLastTile - (tilesInFinestLevel % writeBufferTileSize) * _destTileByteSize;
    }
    ...
    for(uint64_t relIterationId = tilesToIterate - 1; /*relIterationId >= 0*/; --relIterationId){
        ...
        bufferBitmap.smearHorizontal(offsetBufferX + readWidth - 1, offsetBufferY,
                                     ...
                                     std::min<size_t>(_padding, bufferPxWidth - offsetBufferX - readWidth), readHeight);
        ...
        bufferBitmap.smearVertical(0, offsetBufferY + readHeight - 1,
                                   ...
                                   bufferPxWidth, std::min<size_t>(_padding, bufferPxHeight - offsetBufferY - readHeight));
        ...
        for(auto relBufferId = (size_t)(tilesInBuffer - 1); /*relBufferId >= tilesInBuffer*/; --relBufferId) {
            ...
            writeBitmap.copyRectFrom(bufferBitmap,
                                     (size_t)bufferTileX * _innerTileWidth, (size_t)bufferTileY * _innerTileHeight,
                                     0, 0,
            ...
            if(absTileY == (_imageTileHeight - 1)){
                ...
                writeBitmap.fillRect(transPx, Bitmap::PIXEL_FORMAT::RGBA8,
                                     ...
                                     padWidth,
                                     _tileHeight - ((_imageHeight - 1) % _innerTileHeight) - 1 - (_padding << 1));
            }
...
#ifdef  PREPROCESSOR_LOG_PROGRESS
    std::cout << "Done in " << std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - start).count() << " ms." << std::endl << std::endl;
#endif
}
    ...
    switch(_destLayout){
        ...
        case AtlasFile::LAYOUT::PACKED:
            ...
            for(uint64_t i = 0; i < count; ++i) {
                ...
                if (slot > id || absId > firstAvailableId || absId < lastAvailableId || !_offsetIndex->exists(absId)) {
...
void Preprocessor::_calcDeltaE(size_t maxMemory){
    size_t innerTilePxSize = _innerTileWidth * _innerTileHeight;
    auto buffer = new double[innerTilePxSize * _treeDepth];
    size_t tilesInBuffer = maxMemory / _destTileByteSize;
    size_t tileBufferSize = tilesInBuffer * _destTileByteSize;
    auto tileBuffer = new uint8_t[tileBufferSize];
    auto loadBuffer = new uint8_t[_destTileByteSize];
    uint64_t firstIdInBuffer = UINT64_MAX;
    uint64_t lastIdInBuffer = UINT64_MAX;

    Bitmap loadBitmap(_tileWidth, _tileHeight, _destPxFormat, tileBuffer);

    size_t labPxByteSize = 12;
    size_t labTileSize = innerTilePxSize * labPxByteSize;
    auto labRootBitmapBuffer = new uint8_t[labTileSize * 2];
    auto labLeafBitmapBuffer = &labRootBitmapBuffer[labTileSize];

    Bitmap labRootBitmap(_innerTileWidth, _innerTileHeight, Bitmap::PIXEL_FORMAT::LAB, labRootBitmapBuffer);
    Bitmap labLeafBitmap(_innerTileWidth, _innerTileHeight, Bitmap::PIXEL_FORMAT::LAB, labLeafBitmapBuffer);
    uint64_t levelTileWidth = ((_imageTileWidth + 1) >> 1);
    uint64_t levelTileHeight = ((_imageTileHeight + 1) >> 1);

    for(size_t level = _treeDepth - 2; ; --level){
        uint64_t firstId = QuadTree::firstIdOfLevel(level);
        uint64_t levelWidth = QuadTree::getWidthOfLevel(level);
        uint64_t tilesCount = levelWidth * levelWidth;
        uint64_t actualTilesCount = levelTileWidth * levelTileHeight;
        ...
        std::cout << "Calculating Delta-E for " << (actualTilesCount) << " Tiles in Level " << level << std::endl;
        std::cout << std::setw(3) << (int)progress << " %";
        std::cout.flush();
#endif

        uint64_t iterTreeDepth = _treeDepth - level;

        for(uint64_t relId = tilesCount - 1; ; --relId){
            uint64_t absId = firstId + relId;
            uint64_t x;
            uint64_t y;

            QuadTree::getCoordinatesInLevel(relId, level, x, y);

            if(x >= levelTileWidth || y >= levelTileHeight){
                continue;
            }

            loadBitmap.setData(loadBuffer);
            auto len = _loadTileById(absId, loadBitmap.getData());

            if(len == 0){
                std::memset(loadBitmap.getData(), 0, _destTileByteSize);
            }

            labRootBitmap.copyRectFrom(loadBitmap, _padding, _padding, 0, 0, _innerTileWidth, _innerTileHeight);

            size_t iterLevel = iterTreeDepth - 1;
            uint64_t iterLevelWidth = QuadTree::getWidthOfLevel(iterLevel);
            uint64_t iterTilesCount = iterLevelWidth * iterLevelWidth;

            for(uint64_t relIterId = iterTilesCount - 1; ; --relIterId){
                uint64_t absIterId = firstId + relId * iterTilesCount + relIterId;
                uint64_t currentId = relIterId;
                size_t currentLevel = iterLevel;

                if(absIterId < lastIdInBuffer) {
                    firstIdInBuffer = absIterId;

                    if((tilesInBuffer - 1) > firstIdInBuffer){
                        lastIdInBuffer = 0;
                    }else{
                        lastIdInBuffer = firstIdInBuffer + 1 - tilesInBuffer;
                    }

                    _loadToSeqBuffer(tileBuffer, firstIdInBuffer, firstIdInBuffer - lastIdInBuffer + 1);

                    for(size_t i = 0; i < tileBufferSize; ++i){
                        std::cout << (int)tileBuffer[i];
                    }
                }

                loadBitmap.setData(&tileBuffer[(firstIdInBuffer - absIterId) * _destTileByteSize]);
                labLeafBitmap.copyRectFrom(loadBitmap, _padding, _padding, 0, 0, _innerTileWidth, _innerTileHeight);

                uint64_t xInLevel;
                uint64_t yInLevel;

                QuadTree::getCoordinatesInLevel(relIterId, iterLevel, xInLevel, yInLevel);

                uint64_t xOffset = xInLevel * _innerTileWidth / iterLevelWidth;
                uint64_t yOffset = yInLevel * _innerTileHeight / iterLevelWidth;
                ...
                for (size_t y = 0; y < _innerTileHeight; ++y) {
                    for (size_t x = 0; x < _innerTileWidth; ++x) {
                        auto rootX = xOffset + (x >> iterLevel);
                        auto rootY = yOffset + (y >> iterLevel);

                        auto rootPx = &((float *) labRootBitmapBuffer)[(rootY * _innerTileWidth + rootX) * 3];
                        auto leafPx = &((float *) labLeafBitmapBuffer)[(y * _innerTileWidth + x) * 3];

                        deltaL = rootPx[0] - leafPx[0];
                        deltaA = rootPx[1] - leafPx[1];
                        deltaB = rootPx[2] - leafPx[2];

                        buffer[innerTilePxSize * currentLevel + y * _innerTileWidth + x] =
                                std::sqrt(deltaL * deltaL +
                                          deltaA * deltaA +
                                          deltaB * deltaB);
                    }
                }
                ...
                while((currentId & 3) == 3) {
                    auto readBuffer = &buffer[innerTilePxSize * currentLevel];
                    auto writeBuffer = &buffer[innerTilePxSize * (currentLevel - 1)];
                    auto xOffset = (uint8_t)(currentId & 1) * (_innerTileHeight >> 1);
                    auto yOffset = (uint8_t)((currentId & 2) >> 1) * (_innerTileWidth >> 1);

                    for (size_t y = 0; y < _innerTileHeight; y += 2) {
                        for (size_t x = 0; x < _innerTileWidth; x += 2) {
                            writeBuffer[((y >> 1) + yOffset) * _innerTileWidth + (x >> 1) + xOffset] =
                                    ((readBuffer[y * _innerTileWidth + x] +
                                      readBuffer[y * _innerTileWidth + x + 1]) / 2 +
                                     (readBuffer[(y + 1) * _innerTileWidth + x] +
                                      readBuffer[(y + 1) * _innerTileWidth + x + 1]) / 2) / 2;
                        }
                    }
                    ...
                    if(currentLevel == 0){
                        size_t num = (innerTilePxSize >> 1);

                        while(num > 0) {
                            for(size_t i = 0; i < num; ++i){
                                readBuffer[i] = (readBuffer[i << 1] + readBuffer[(i << 1) + 1]) / 2;
                            }

                            num >>= 1;
                        }

                        _cielabIndex->set(absId, (float)readBuffer[0]);
                ...
                auto currentProgress = (uint8_t)(tilesLoaded * 100 / (levelTileWidth * levelTileHeight * iterTilesCount));
        ...
        firstIdInBuffer = UINT64_MAX;
        lastIdInBuffer = UINT64_MAX;
        levelTileWidth = ((levelTileWidth + 1) >> 1);
        levelTileHeight = ((levelTileHeight + 1) >> 1);
    }

    delete labRootBitmapBuffer;
    delete tileBuffer;
